...
function initOmniChat() {
  ...
  /* SEGURIDAD: CFG.token es visible en el código fuente del navegador y NO
     debe considerarse un secreto. La autenticación y validación real del
     origen deben implementarse en n8n, en un proxy o en el backend. */
  const DEBUG = false;
  ...
  function renderMarkdown(text, container) {
    ...
    while ((match = TOKEN_RE.exec(text)) !== null) {
      ...
      if (match[0] === '\n') {
        ...
        a.textContent = label;
        a.style.cssText = 'color:#C050C0;font-weight:600;text-decoration:underline;cursor:pointer;';
        a.href = '#contacto';
  ...
  function checkAbuse(text) {
    if (text.length > CFG.maxMsgLength) {
      return `Tu mensaje es demasiado largo. Por favor, acórtalo un poco (máximo ${CFG.maxMsgLength} caracteres).`;
    }
    const now = Date.now();
    if (now - lastSendTime < CFG.cooldownMs) {
      return 'Espera un momento antes de enviar otro mensaje.';
    }
    sessionTimestamps = sessionTimestamps.filter(t => now - t < CFG.rateWindow);
    if (sessionTimestamps.length >= CFG.rateMax) {
      return 'Has enviado demasiados mensajes en poco tiempo. Por favor, espera unos minutos e inténtalo de nuevo.';
    }
    return null;
  }
  ...
  async function sendMessage(text) {
    ...
    if (abuse) { addMessage(abuse, 'bot'); return; }
    ...
    try {
      ...
      if (DEBUG) console.error('[OmniChat]', err);
      typingEl.remove();
