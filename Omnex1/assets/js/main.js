...
/* En móvil táctil se usa <div> con animación CSS (WebGL falla en Safari/Android) */
const _isTouchOnly = window.matchMedia('(hover: none) and (pointer: coarse)').matches;
document.querySelectorAll('.problem-card, .trust-card, .industry-card').forEach(el => {
  const bg = document.createElement(_isTouchOnly ? 'div' : 'canvas');
...
(function () {
  if (_isTouchOnly) return; /* móvil: CSS animation en su lugar */
  const allCanvases = document.querySelectorAll('.plan__canvas');
  if (!allCanvases.length) return;

  const PRM = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  /* Canvas WebGL compartido — debe estar en el DOM para que iOS Safari
     permita leer sus píxeles vía drawImage (restricción conocida de WebKit) */
  const SIZE = 256;
